...
- [x] В блоке `slotsError` добавить кнопку повтора:
  ```tsx
  ...
  ```
  Retry реализован через `retryTrigger` атом в модели — инкремент перезапускает computed без закрытия модалки.
- [x] Запустить тесты + type-check
- [x] Сделать коммит: `feat: добавить кнопку повтора при ошибке загрузки слотов`

---

## Финальная проверка

- [x] Запустить полный набор тестов: `pnpm vitest run`
- [x] Запустить сборку: `pnpm build`
- [x] Проверить вручную в браузере (mock-режим): `pnpm start:mock` (skipped - manual test, not automatable)

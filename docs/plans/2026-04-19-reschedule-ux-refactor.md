...
- [x] Заменить блок `Хотите внести изменения?` на группу явных кнопок:
  ```tsx
  ...
  ```
- [x] Убедиться что кнопки отображаются только при `booking.status === 'confirmed'`
- [x] Запустить тесты: `pnpm vitest run` + `pnpm type-check`
- [x] Сделать коммит: `feat: заменить инлайн-ссылки переноса/отмены на кнопки`
...
- [x] В `RescheduleModal.tsx` при отображении заголовка дня изменить формат:
  ```tsx
  // было:
  {formatDate(day)}
  // стало:
...
- [x] Проверить что dayjs настроен с русской локалью (уже есть в `app/`)
- [x] Запустить тесты + type-check
- [x] Сделать коммит: `feat: добавить день недели к датам в модалке переноса`
...
- [x] Добавить горизонтальный ряд таблеток-фильтров над `ScrollArea`:
  ```tsx
  ...
  ```
- [x] Добавить локальный `useState<string | null>` для `selectedDay` в компоненте
  (или Reatom-атом, если предпочтительно без React state)
- [x] По умолчанию `selectedDay = null` → показывать все дни
- [x] При выборе таблетки — показывать только слоты этого дня (фильтровать `sortedDays`)
- [x] Сбрасывать `selectedDay` при закрытии модалки (в `close` action)
- [x] Запустить тесты + type-check
- [x] Сделать коммит: `feat: добавить фильтрацию по дате в модалке переноса`
...
- [x] Под `ScrollArea` добавить условный блок, если `selectedSlotId` заполнен:
  ```tsx
...
- [x] Запустить тесты + type-check
- [x] Сделать коммит: `feat: показывать выбранный слот перед подтверждением переноса`
